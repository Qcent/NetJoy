...
class Instrumentor
{
    std::string     m_sessionName   = "Unnamed";
...
public:
    ...
    void beginSession(const std::string& name = "profile", const std::string& filepath = "results.json")
    {
    ...
    void writeHeader()
    {
        m_outputStream << "{\"otherData\": [{\"session\":\"" << m_sessionName << "\"}], \"traceEvents\":[";
    }
...
public:
    ...
    InstrumentationTimer(const std::string & name = "unknown")
        : m_result({ name, 0, 0, 0 })
        , m_stopped(false)
    {
